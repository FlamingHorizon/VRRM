        ...
        for story_id in range(n_samples):
            ...
            for sim_round in range(params['story_sims_per_epoch']):          
                ...
                if (epoch_id + 1) % params['print_epoch_period'] == 0:
                    print '=======simulation====== epoch = %d, story_id = %d, sim_round = %d' %(epoch_id, story_id, sim_round)
                ...
                for time_slice in range(params['max_story_len']):    
                    ...
                    if story_mask_train[train_trace_id, time_slice, 0, 0] > 0:
                        # retrieve the table
                        arg_1_int_first = work_space_table.retr_insert(arg_1_first, inv_vocab_text)
                        arg_2_int_first = work_space_table.retr_insert(arg_2_first, inv_vocab_text)
                        arg_1_int_second = work_space_table.retr_insert(arg_1_second, inv_vocab_text)
                        arg_2_int_second = work_space_table.retr_insert(arg_2_second, inv_vocab_text)

                        working_memory.modifyEnv((arg_1_int_first, arg_r, arg_2_int_first))
                        working_memory.modifyEnv((arg_1_int_second, arg_r, arg_2_int_second))
...
if __name__ == '__main__':
    train_file_names = ['../en/qa12_conjunction_train.txt']
    test_file_names = ['../en/qa12_conjunction_test.txt']
