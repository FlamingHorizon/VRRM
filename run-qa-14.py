...
def DRL_Reasoner(params):
    ...
    embed_seq_q = TimeDistributed(embed_word_q, input_shape=(max_q_num, max_sent_len), name='embed_seq_q')
    ...
    encode_q = TimeDistributed(encode_single_q, input_shape=(max_q_num, max_sent_len, dim_emb_story), name='encode_q')
    q_embs = encode_q(tb_m_emb_q) # (, q_num, dim_emb)
...
class working_environment(object):
    def __init__(self, num_entities, num_relations):
        ...
        self.tempEntiList = []
        self.posList = (-1) * numpy.ones((4, 4)) # not necessary but simplify notifications.
        self.nameDict = {'mary': 0, 'bill': 1, 'julie': 2, 'fred':3}
    ...
    def retrieveRelation(self,arg1, arg2, var_dict):
        hit_list = []
        for i in range(4):
            if self.posList[self.nameDict[var_dict.inv_retr(arg1)], i] == arg2:
                hit_list.append(i)
        if len(hit_list) == 0:
            return -1
        else:
            return_list = []
            for i in range(hit_list[-1]):
                if self.posList[self.nameDict[var_dict.inv_retr(arg1)], i] > 0:
                    return_list.append(i)
            if len(return_list) == 0:
                return -1
            else:
                return int(self.posList[self.nameDict[var_dict.inv_retr(arg1)], return_list[-1]])
        return -1
    ...
    def modifyEnv(self, commend, var_dict):
        ''' commend is a list :(e1, relation, e2), with e1 a new entity by default.'''
        if commend:
            self.posList[self.nameDict[var_dict.inv_retr(commend[0])], commend[1] - 1] = commend[2]
            for i in [0,2]:
...
ent_list = {'1':'mary', '2':'bill', '3':'julie', '4':'fred', '6':'bedroom',
            '7':'kitchen', '8':'cinema', '9':'office', '10':'school', '11':'park'}
            ...
            for sim_round in range(params['story_sims_per_epoch']):
                ...
                for time_slice in range(params['max_story_len']):
                    ...
                    if arg_1 > 0 and arg_2 > 0 and story_mask_train[train_trace_id, time_slice, 0, 0] > 0:
                        ...
                        working_memory.modifyEnv((arg_1_int, arg_r, arg_2_int), work_space_table)
                        if story_id == 3:
                            print (arg_1_int, arg_r, arg_2_int)
                ...
                for q_idx in range(len(retrieve_probs[0])):                    
                    ...
                    if len(ents_in_sent) > 0:
                        ...
                        search_result = working_memory.retrieveRelation(arg1_retr_int, arg2_retr_int, work_space_table)
...
def test_model(simulator, story_int, story_mask, q_int, q_mask, ans_int, ans_mask, params, old_std, f_test, vocab_text, inv_vocab_text):
    ...
    mask_sim = numpy.zeros((1, params['max_story_len_test'], params['dim_emb_story']))
...
def ut_env():
    ...
    b = varTable()
    b.var_dict = {'bill':1, 'fred':3, 'mary':6, 'julie':8}
    a.modifyEnv((1,2,2), b)
    print a.posList
    a.modifyEnv((3,2,2), b)
    print a.posList
    ...
    a.modifyEnv((1,1,5), b)
    print a.posList
    print a.retrieveRelation(3,2, b)
    a.modifyEnv((3,3,4), b)
    print a.posList
    a.modifyEnv((6,1,4), b)
    print a.posList
    ...
    a.modifyEnv((1,3,5), b)
    print a.posList
    print a.retrieveRelation(3,7, b)
    a.modifyEnv((1,4,7), b)
    print a.posList
    a.modifyEnv((8,1,4), b)
    print a.posList
...
if __name__ == '__main__':
    train_file_names = ['../en/qa14_time-reasoning_train.txt']
    test_file_names = ['../en/qa14_time-reasoning_test.txt']
