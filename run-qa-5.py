...
class working_environment(object):
    ...
    def retrieveArg1(self, arg1, arg2):
        return_list = []
        for a, r, b in self.tupleList:
            if r == arg1 and b == arg2:
    ...
    def retrieveArg2Single(self, arg1):
        return_list = []
        for a, r, b in self.tupleList:
            if r == arg1:
                return_list.append(b)
    ...
    def retrieveArg1Single(self, arg1):
        return_list = []
        for a, r, b in self.tupleList:
            if r == arg1:
                return_list.append(a)
...
ent_list = {'1':'office', '2':'hallway', '3':'kitchen', '4':'garden', '5':'bedroom', '6':'bathroom',
            '7':'mary', '8':'fred', '9':'bill', '10':'jeff', '11':'milk','12':'football','13':'apple'}
            ...
            for sim_round in range(params['story_sims_per_epoch']):
                ...
                for time_slice in range(params['max_story_len']):
                    ...
                    if arg_1 > 0 and arg_2 > 0 and story_mask_train[train_trace_id, time_slice, 0, 0] > 0:
                        ...
                        if arg_r == 1:
                            if len(ents_in_sent) < 3:
                                working_memory.modifyEnv((arg_1_int, -1, arg_2_int))
                            else:
                                working_memory.modifyEnv((arg_1_int, -1, arg_2_int))
                                working_memory.modifyEnv((arg_3_int, -1, arg_2_int))
                        else:
                            if len(ents_in_sent) < 3:
                                working_memory.modifyEnv((arg_1_int, arg_2_int, arg_2_int))
                            else:
                                working_memory.modifyEnv((arg_1_int, arg_2_int, arg_3_int))
                ...
                for q_idx in range(len(retrieve_probs[0])):                    
                    ...
                    if len(ents_in_sent) == 2:
                        ...
                        if relation_pred == 1:
                            ...
                            search_result = working_memory.retrieveArg1(arg1_retr_int, arg2_retr_int)
                    else:
                        arg1_retr = int(ents_in_sent[0])
                        arg1_retr_int = work_space_table.retr(arg1_retr, inv_vocab_text)
                        if relation_pred == 1:
                            ...
                            search_result = working_memory.retrieveArg2Single(arg1_retr_int) # who received the milk?
                        else:
                            search_result = working_memory.retrieveArg1Single(arg1_retr_int) # who gave the milk?
...
if __name__ == '__main__':
    train_file_names = ['../en/qa5_three-arg-relations_train.txt']
    test_file_names = ['../en/qa5_three-arg-relations_test.txt']
