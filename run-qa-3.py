...
def parse_stories(lines, vocab, inv_vocab, cut):
    ...
    for line in lines:
        ...
        if q_flag == 1 and line_count != 0:
            support_int = [int(sent_id) for sent_id in supporting.strip().split(' ')]
            start_id = min(support_int)
            end_id = max(support_int)
            if cut == 0 or (cut == 1 and end_id - start_id + 1 <= 8):
                substory = story[start_id - 1: end_id]
                substory = [x for x in substory if x]
    ...
    def modifyEnv(self, commend):
        ...
        if commend:
            ...
            if commend[1] == 1: # not-relative
                prev_position = [t for t in self.tupleList if t[0] == commend[0] and t[1] == -1]
                if len(prev_position) == 0:
                    self.tupleList.append((commend[0], -1, commend[2]))
                    move_b = []
                    for a, r, b in self.tupleList:
                        if a == commend[0] and r == -2:
                            move_b.append((b, -1, commend[2]))
                    for a, r, b in move_b:
                        self.tupleList = [t for t in self.tupleList if not(t[0] == a and t[1] == -1)]
                    self.tupleList.extend(move_b)
                else:
                    prev_id = prev_position[0][2]
                    # update people state
                    self.tupleList = [t for t in self.tupleList if not(t[0] == commend[0] and t[1] == -1)]
                    self.tupleList.append((commend[0], -1, commend[2]))
                    self.tupleList.append((commend[0], prev_id, commend[2]))
                    # update object state
                    move_b = []
                    move_extend_b = []
                    for a, r, b in self.tupleList:
                        if a == commend[0] and r == -2:
                            move_b.append((b, -1, commend[2]))
                    for a, r, b in move_b:
                        prev_obj_position = [t for t in self.tupleList if t[0] == a and t[1] == -1]
                        if len(prev_obj_position) == 0:
                            move_extend_b.append((a, -1, commend[2]))
                        else:
                            prev_obj_id = prev_obj_position[-1][2]
                            move_extend_b.append((a, -1, commend[2]))
                            self.tupleList = [t for t in self.tupleList if not(t[0] == a and t[1] == -1)]
                            move_extend_b.append((a, prev_obj_id, commend[2]))
                    self.tupleList.extend(move_extend_b)
                    
            elif commend[1] == 2:
                if commend in self.tupleList:
                    self.tupleList = []
                else:
                    self.tupleList.append((commend[0], -2, commend[2])) # a people has a belonging
                    move_to = None
                    for a, r, b in self.tupleList:
                        if a == commend[0] and r == -1:
                            move_to = (commend[2], -1, b)
                    if move_to:
                        self.tupleList = [t for t in self.tupleList if not(t[0] == move_to[0] and t[1] == -1)]
                        self.tupleList.append(move_to)                
            else:
                temp_tupleList = [t for t in self.tupleList if not(t[0] == commend[0] and t[1] == -2 and t[2] == commend[2])]
                if 0:
                    self.tupleList = []
                else:
                    self.tupleList = temp_tupleList
                    move_to = []
                    for a, r, b in self.tupleList:
                        if a == commend[0]:
                            move_to.append((commend[2], r, b))
                    for m_t in move_to:
                        self.tupleList = [t for t in self.tupleList if not(t[0] == move_to[0] and t[1] == -1)]
                        self.tupleList.append(m_t)
...
ent_list = {'1':'mary', '2':'sandra', '3':'john', '4':'daniel', '5':'bathroom', '6':'bedroom',
            '7':'kitchen', '8':'garden', '9':'office', '10':'hallway', '11':'football', '12':'apple', '13':'milk'}
...
def set_hyper_params():
    ...
    params['enti_num'] = 20 
    params['relation_num'] = 3
    params['relation_num_retr'] = 1
...
if __name__ == '__main__':
    train_file_names = ['../en/qa3_three-supporting-facts_train.txt']
    test_file_names = ['../en/qa3_three-supporting-facts_test.txt']
