    ...
    def modifyEnv(self, commend):
        ...
        if commend:
            ...
            if commend[1] == 1:
                ...
                self.tupleList = [t for t in self.tupleList if not(t[0] == commend[0] and t[2] == commend[2] and t[1] == 1)]
            else:
                self.tupleList = []
...
def compute_single_reward_list(final_ans,label_word):
    reward_scalar = -0.1
    label_list = label_word.split(',')
    flag = 1
    if len(final_ans) == 0:
        flag = 0
    for w in final_ans:
...
ent_list = {'1':'office', '2':'hallway', '3':'kitchen', '4':'garden', '5':'bedroom', '6':'bathroom',
            '7':'mary', '8':'john', '9':'daniel', '10':'sandra',
            '11':'milk', '12':'football','13': 'apple'}
        ...
        for story_id in range(n_samples):
            ...
            for sim_round in range(params['story_sims_per_epoch']):
                ...
                for time_slice in range(params['max_story_len']):
                    ...
                    if arg_1 > 0 and arg_2 > 0 and story_mask_train[train_trace_id, time_slice, 0, 0] > 0:
                        if len(ents_in_sent) < 3:
                            ...
                            if arg_r <= 2:
                                working_memory.modifyEnv((arg_1_int, 4, arg_2_int))
                                working_memory.modifyEnv((arg_3_int, 4, arg_2_int))
                            else:
                                working_memory.modifyEnv((arg_1_int, 2, arg_2_int))
                                working_memory.modifyEnv((arg_3_int, 1, arg_2_int))
                ...
                for q_idx in range(len(retrieve_probs[0])):                    
                    ...
                    count_result_list = working_memory.retrieveArgList(arg1_retr_int)                  
                    
                    retrieved_relation_list.append(count_result_list)
                ...
                for q_idx in range(len(retrieve_probs[0])):
                    ...
                    if len(retrieved_relation_list[q_idx]) > 0:
                        ans_pred = [work_space_table.inv_retr(w) for w in retrieved_relation_list[q_idx]]
                    else:
                        ans_pred = ['nothing']
                    reward_scalar_q, ans_q, label_q = compute_single_reward_list(ans_pred,ans_word)
            ...
            if arg_1 > 0 and arg_2 > 0 and story_mask_test[train_trace_id, time_slice, 0, 0] > 0:
                if len(ents_in_sent) < 3:
                    ...
                    if arg_r <= 2:
                        ...
                        working_memory.modifyEnv((arg_1_int, 2, arg_2_int))
                        working_memory.modifyEnv((arg_3_int, 1, arg_2_int))
                    story_commands.append((arg_1_int, arg_2_int, arg_3_int, arg_r))
...
if __name__ == '__main__':
    train_file_names = ['../en/qa8_lists-sets_train.txt']
    test_file_names = ['../en/qa8_lists-sets_test.txt']
