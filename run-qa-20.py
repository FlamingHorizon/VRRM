...
class working_environment(object):
    ...
    def modifyEnv(self, commend, vocab, inv_vocab, var_dict):
        solution_dict = {'hungry':'kitchen', 'thirsty':'kitchen', 'bored':'garden', 'tired':'bedroom'}
        if commend:
            # add the origin relationship into tupleList
            if commend[1] == 1: # body-state
                self.tupleList = [t for t in self.tupleList if not(t[0] == commend[0] and t[1] == 1)]
                state_key = var_dict.inv_retr(commend[2])
                if state_key in solution_dict:
                    solution_str = solution_dict[state_key]
                    solution_spaceid = var_dict.retr(vocab[solution_str], inv_vocab)
                    self.tupleList = [t for t in self.tupleList if not(t[0] == commend[0] and t[1] == 2)]
                    self.tupleList.append((commend[0], 2, solution_spaceid)) 
...
ent_list = {'1':'sumit', '2':'yann', '3':'jason', '4':'antoine', '5':'tired', '6':'bored',
            '7':'hungry', '8':'thirsty', '9':'bedroom', '10':'pajamas', '11':'kitchen', '12':'football', '13':'milk',
            '14':'apple', '15': 'garden'}
    ...
    for epoch_id in range(params['epoch_num']):
        ...
        for story_id in range(n_samples):
            ...
            for sim_round in range(params['story_sims_per_epoch']):
                ...
                work_space_table.retr_insert(vocab_text['kitchen'], inv_vocab_text)
                work_space_table.retr_insert(vocab_text['garden'], inv_vocab_text)
                work_space_table.retr_insert(vocab_text['bedroom'], inv_vocab_text)
                ...
                for time_slice in range(params['max_story_len']):
                    ...
                    if arg_1 > 0 and arg_2 > 0 and story_mask_train[train_trace_id, time_slice, 0, 0] > 0:
                        ...
                        working_memory.modifyEnv((arg_1_int, arg_r, arg_2_int), vocab_text, inv_vocab_text, work_space_table)
...
if __name__ == '__main__':
    train_file_names = ['../en/qa20_agents-motivations_train.txt']
    test_file_names = ['../en/qa20_agents-motivations_test.txt']
