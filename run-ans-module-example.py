...
def DRL_Reasoner(params):
    ...
    ans_input_dim = params['ans_input_dim']
    ...
    ans_input = Input(shape=(max_q_num, ans_input_dim))
    ...
    reward_value_ans = Input(shape=(max_q_num, vocab_size_ans))
    ...
    # answer module
    hidden_ans = TimeDistributed(Dense(dim_q_h_ff, activation='sigmoid'), input_shape=(max_q_num, ans_input_dim))(ans_input)
    ans_probs = Activation('softmax')(Tho(params['tho'])(TimeDistributed(Dense(vocab_size_ans))(hidden_ans)))
    ans_probs_log = Lambda(lambda x: (-1) * (K.log(x) + K.epsilon()))(ans_probs)
    ans_probs_re = merge([ans_probs_log, reward_value_ans], mode='mul', name='ans_probs_re')

    # a complete model
    DRL_complete = Model(input=[story_input, story_mask, q_input, q_mask, ans_input, mask_sim, reward_value, reward_value_retr, reward_value_ans],
                                                output=[bind_probs_re, retrieve_probs_re, ans_probs_re])
    rmsp = RMSprop(clipnorm=2., lr=0.0001)
    sgd = SGD(clipnorm=2.)
    adad = Adadelta(clipnorm=10.)
    DRL_complete.compile(optimizer=rmsp,
              loss={'action_probs_re': dot_loss, 'retrieve_probs_re': dot_loss, 'ans_probs_re': dot_loss},)
    DRL_sim = Model(input=[story_input, story_mask, q_input, q_mask, ans_input, mask_sim, reward_value, reward_value_retr, reward_value_ans],
                                                output=[bind_probs, retrieve_probs, ans_probs])
...
class working_environment(object):
    ...
    def modifyEnv(self, commend):
        ...
        if commend:
            ...
            if commend[1] == 1:
                self.tupleList = [t for t in self.tupleList if not(t[0] == commend[0])]
...
ent_list = {'1':'office', '2':'hallway', '3':'kitchen', '4':'garden', '5':'bedroom', '6':'bathroom',
            '7':'mary', '8':'sandra', '9':'daniel', '10':'john',
            '11':'apple', '12':'football', '13':'milk'
           }
...
def train_two_phrase(train_file_names, test_file_names, params):
    ...
    params['ans_input_dim'] = 2
    params['vocab_size_ans'] = 2
    ...
    ans_input = numpy.zeros((n_samples * params['story_sims_per_epoch'], params['max_q_num'], params['ans_input_dim']))
    ...
    fin_one_hot_ans_train = numpy.zeros((n_samples * params['story_sims_per_epoch'], params['max_q_num'], params['vocab_size_ans']))
    reward_fin_ans_train = numpy.ones((n_samples * params['story_sims_per_epoch'], params['max_q_num'], params['vocab_size_ans']))
    ...
    for epoch_id in range(params['epoch_num']):
        ...
        for story_id in range(n_samples):
            ...
            for sim_round in range(params['story_sims_per_epoch']):
                ...
                action_probs, retrieve_probs, pred_probs = simulator.predict([story_train[numpy.newaxis,train_trace_id], 
                                                                      ...
                                                                      ans_input[numpy.newaxis, train_trace_id],
                                                                      mask_sim[:], 
                                                                      reward_train[numpy.newaxis,train_trace_id],
                                                                      reward_fin_train[numpy.newaxis,train_trace_id],
                                                                      reward_fin_ans_train[numpy.newaxis,train_trace_id]])
                for time_slice in range(params['max_story_len']):
                ...
                for q_idx in range(len(retrieve_probs[0])):                    
                    ...
                    search_result = working_memory.retrieveRelation(arg1_retr_int, arg2_retr_int)                  
                    
                    if search_result == 1:
                        ans_input[train_trace_id, q_idx, :] = numpy.array([1,0])
                    else:
                        ans_input[train_trace_id, q_idx, :] = numpy.array([0,1])
                ...
                for q_idx in range(len(retrieve_probs[0])):
                    ...
                    ans_idx, action_one_hot_ans = select_action(pred_probs[0,q_idx,:], params['epsilon'])
                    ...
                    fin_one_hot_ans_train[train_trace_id, q_idx,:] = action_one_hot_ans
                        
                    reward_scalar_q, ans_q, label_q = compute_single_reward(ans_pred,ans_word)
                ...
                for q_idx in range(len(retrieve_probs[0])):
                    ...
                    reward_fin_ans_train[train_trace_id, q_idx, :] *= (reward_q_total * mask_reward)
        ...
        reasoner.fit([story_train, story_mask_train, q_train, q_mask_train, ans_input, mask_sim_2, reward_train, reward_fin_train, reward_fin_ans_train],
                      {'action_probs_re': act_selected_train, 'retrieve_probs_re': fin_one_hot_train, 'ans_probs_re': fin_one_hot_ans_train },
...
def test_model(simulator, story_int, story_mask, q_int, q_mask, ans_int, ans_mask, params, old_std, f_test, vocab_text, inv_vocab_text):
    ...
    ans_input_test = numpy.zeros((n_samples * params['story_sims_per_epoch'], params['max_q_num'], params['ans_input_dim']))
    ...
    fin_one_hot_test = numpy.zeros((1, params['max_q_num_test'], (params['ent_range']) * 1 * params['relation_num'])) # one hot arg1 arg2 r_pred
    reward_fin_test = numpy.ones((1, params['max_q_num_test'], (params['ent_range']) * 1 * params['relation_num']))
    fin_one_hot_ans_test = numpy.ones((1, params['max_q_num_test'], params['vocab_size_ans']))
    reward_fin_ans_test = numpy.ones((1, params['max_q_num_test'], params['vocab_size_ans']))
    ...
    for story_id in range(n_samples):
        ...
        for time_slice in range(params['max_story_len_test']): 
            ...
            if arg_1 > 0 and arg_2 > 0 and story_mask_test[train_trace_id, time_slice, 0, 0] > 0:
                if len(ents_in_sent) < 3:
                    ...
                    if arg_r <= 2:
                        working_memory.modifyEnv((arg_1_int, arg_r, arg_2_int))
                        story_commands.append((arg_1_int, arg_r, arg_2_int))
                else:
                    arg_1_int = work_space_table.retr_insert(arg_1, inv_vocab_text)
                    arg_2_int = work_space_table.retr_insert(arg_2, inv_vocab_text)
                    arg_3_int = work_space_table.retr_insert(arg_3, inv_vocab_text)
        ...
        for q_idx in range(len(retrieve_probs[0])):                    
            ...
            if search_result == 1:
                ans_input_test[train_trace_id, q_idx, :] = numpy.array([1,0])
            else:
                ans_input_test[train_trace_id, q_idx, :] = numpy.array([0,1])
            if q_idx == 0:
                retrieve_state.append(ans_input_test[train_trace_id, q_idx,:])
        ...
        for q_idx in range(len(retrieve_probs[0])):
            ...
            ans_idx, action_one_hot_ans = select_action_hard(pred_probs[0,q_idx,:], params['epsilon'])
            ...
            fin_one_hot_ans_test[train_trace_id, q_idx,:] = action_one_hot_ans
            reward_scalar_q, ans_q, label_q = compute_single_reward(ans_pred,ans_word)
        ...
        for q_idx in range(len(retrieve_probs[0])):
            ...
            reward_fin_ans_test[train_trace_id, q_idx, :] *= reward_q_total
...
if __name__ == '__main__':
    train_file_names = ['../en/qa6_yes-no-questions_train.txt']
    test_file_names = ['../en/qa6_yes-no-questions_test.txt']
